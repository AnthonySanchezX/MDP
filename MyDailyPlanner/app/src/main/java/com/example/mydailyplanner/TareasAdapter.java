...
public class TareasAdapter extends BaseAdapter {
    ...
    public TareasAdapter(Activity a, ArrayList<HashMap<String, String>> d, BD mydb) {
        activity = a;
        data = d;
        database = mydb;
    }
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
            ...
            convertView = LayoutInflater.from(activity).inflate(R.layout.vista_tarea, parent, false);
            holder.task_name = convertView.findViewById(R.id.nombre_tarea);
            holder.checkBtn = convertView.findViewById(R.id.btn_check);
        ...
        try {
            ...
            holder.checkBtn.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
                ...
                public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                    if (isChecked) {
                        database.actualizarStatus(singleTask.get("id"), 1);
                        tmpHolder.task_name.setText(Html.fromHtml("<strike>" + singleTask.get("task") + "</strike>"));
                    } else {
                        database.actualizarStatus(singleTask.get("id"), 0);
